...
var SC_FlexGrid = function(element) {
  ...
  var app = this;
  var promise = ServercideApp.call(this, app, element, "SC_FlexGrid", defaultParams).then(function(){ app.postStrap(app); });
  return promise;
}
...
SC_FlexGrid.prototype.postStrap = function(app){
  app.debugMsg(app.element.attr("id") + " app " + app.getMetaParam("type") + " is strapping, running onStrap before recursion.", 2);
  return new Promise(function(fulfill, reject){
    fulfill();
  });
...
SC_FlexGrid.prototype.discoveryComplete = function(app){
